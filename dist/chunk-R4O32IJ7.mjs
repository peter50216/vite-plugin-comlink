...
var builtins$1 = "Infinity NaN undefined null true false eval uneval isFinite isNaN parseFloat parseInt decodeURI decodeURIComponent encodeURI encodeURIComponent escape unescape Object Function Boolean Symbol Error EvalError InternalError RangeError ReferenceError SyntaxError TypeError URIError Number Math Date String RegExp Array Int8Array Uint8Array Uint8ClampedArray Int16Array Uint16Array Int32Array Uint32Array Float32Array Float64Array Map Set WeakMap WeakSet SIMD ArrayBuffer DataView JSON Promise Generator GeneratorFunction Reflect Proxy Intl".split(" ");
var blacklisted = new Set(reservedWords$1.concat(builtins$1));
var illegalCharacters = /[^$_a-zA-Z0-9]/g;
var startsWithDigit = (str) => /\d/.test(str[0]);
...
function getAliasName(id) {
  ...
  return base2.substr(0, base2.length - extname(id).length);
}
...
var ExternalModule = class {
  ...
  setRenderPath(options, inputBase) {
    ...
    if (!this.renderPath) {
      this.renderPath = this.renormalizeRenderPath ? normalize(relative$1(inputBase, this.id)) : this.id;
    }
...
var ImportDefaultSpecifier$1 = "ImportDefaultSpecifier";
var ImportNamespaceSpecifier$1 = "ImportNamespaceSpecifier";
...
var EVENT_ACCESSED = 0;
var EVENT_ASSIGNED = 1;
var EVENT_CALLED = 2;
...
var FunctionNode = class extends NodeBase {
  constructor() {
    super(...arguments);
...
var ArrowFunctionExpression = class extends NodeBase {
  constructor() {
    super(...arguments);
    this.deoptimizedReturn = false;
  }
...
var CallExpression = class extends NodeBase {
  constructor() {
    super(...arguments);
    this.deoptimized = false;
...
var INTEROP_DEFAULT_LEGACY_VARIABLE = "_interopDefaultLegacy";
...
var ASSET_PREFIX = "ROLLUP_ASSET_URL_";
var CHUNK_PREFIX = "ROLLUP_CHUNK_URL_";
...
var MetaProperty = class extends NodeBase {
  ...
  renderFinalMechanism(code, chunkId, format, outputPluginDriver) {
    ...
    if (metaProperty && (metaProperty.startsWith(FILE_PREFIX) || metaProperty.startsWith(ASSET_PREFIX) || metaProperty.startsWith(CHUNK_PREFIX))) {
      ...
      const relativePath2 = normalize(relative$1(dirname(chunkId), fileName));
      let replacement2;
...
var getRelativeUrlFromDocument = (relativePath2, umd2 = false) => getResolveUrl(`'${relativePath2}', ${umd2 ? `typeof document === 'undefined' ? location.href : ` : ""}document.currentScript && document.currentScript.src || document.baseURI`);
var getGenericImportMetaMechanism = (getUrl) => (prop, chunkId) => {
  const urlMechanism = getUrl(chunkId);
  return prop === null ? `({ url: ${urlMechanism} })` : prop === "url" ? urlMechanism : "undefined";
};
var getUrlFromDocument = (chunkId, umd2 = false) => `${umd2 ? `typeof document === 'undefined' ? location.href : ` : ""}(document.currentScript && document.currentScript.src || new URL('${chunkId}', document.baseURI).href)`;
...
var TaggedTemplateExpression = class extends NodeBase {
  bind() {
    super.bind();
...
var getStartTime = () => [0, 0];
var getElapsedTime = () => 0;
var getMemory = () => 0;
var timers = {};
var normalizeHrTime = (time) => time[0] * 1e3 + time[1] / 1e6;
...
function markModuleAndImpureDependenciesAsExecuted(baseModule) {
  ...
  for (const module of modules) {
    for (const dependency of [...module.dependencies, ...module.implicitlyLoadedBefore]) {
      if (!(dependency instanceof ExternalModule) && !dependency.isExecuted && (dependency.info.hasModuleSideEffects || module.implicitlyLoadedBefore.has(dependency)) && !visitedModules.has(dependency.id)) {
  ...
  constructor(graph, id, options, isEntry, hasModuleSideEffects, syntheticNamedExports, meta) {
    ...
    this.info = {
      ...
      get dynamicallyImportedIds() {
        ...
        for (const { id: id2 } of module.dynamicImports) {
          if (id2) {
    ...
    for (const { skipExternalNamespaces, searchedNamesAndModules: searchedNamesAndModules2 } of skipExternalNamespaceValues) {
      ...
      for (const module of this.exportAllModules) {
        if (module instanceof Module || !skipExternalNamespaces) {
          const declaration = getVariableForExportNameRecursive(module, name, importerForSideEffects, true, searchedNamesAndModules2, skipExternalNamespaces);
          if (declaration) {
...
var thisProp = (name) => `this${keypath(name)}`;
...
var getStarExcludesBlock = (starExcludes, varOrConst, _, t, n2) => starExcludes ? `${n2}${t}${varOrConst} _starExcludes${_}=${_}{${_}${[...starExcludes].map((prop) => `${prop}:${_}1`).join(`,${_}`)}${_}};` : "";
var getImportBindingsBlock = (importBindings, _, t, n2) => importBindings.length ? `${n2}${t}var ${importBindings.join(`,${_}`)};` : "";
...
var getHoistedExportsBlock = (exports, _, t, n2) => getExportsBlock(exports.filter((expt) => expt.hoisted).map((expt) => ({ name: expt.exported, value: expt.local })), _, t, n2);
var getMissingExportsBlock = (exports, _, t, n2) => getExportsBlock(exports.filter((expt) => expt.local === MISSING_EXPORT_SHIM_VARIABLE).map((expt) => ({ name: expt.exported, value: MISSING_EXPORT_SHIM_VARIABLE })), _, t, n2);
var getSyntheticExportsBlock = (exports, _, t, n2) => getExportsBlock(exports.filter((expt) => expt.expression).map((expt) => ({ name: expt.exported, value: expt.local })), _, t, n2);
...
function umd(magicString, { accessedGlobals, dependencies, exports, hasExports, id, indentString: t, intro, namedExportsMode, outro, varOrConst, warn }, { amd: amd2, compact, esModule, extend: extend2, externalLiveBindings, freeze, interop, name, namespaceToStringTag, globals, noConflict, strict }) {
  ...
  if (noConflict) {
    ...
    if (!namedExportsMode && hasExports) {
      ...
      factory = `var ${noConflictExportsVar}${_}=${_}${module};${n2}${t}${t}${factoryVar}(${[noConflictExportsVar].concat(globalDeps).join(`,${_}`)});`;
    }
...
function getCollapsedSourcemap(id, originalCode, originalSourcemap, sourcemapChain, linkMap) {
  ...
  if (!originalSourcemap) {
    ...
    const baseSources = sources.map((source2, i) => new Source(resolve(directory, sourceRoot, source2), sourcesContent[i]));
...
function deconflictImportsEsmOrSystem(usedNames, imports, dependenciesToBeDeconflicted, _interop, preserveModules, _externalLiveBindings, chunkByModule, syntheticExports) {
  ...
  for (const variable of imports) {
    ...
    if (variable.isNamespace && (preserveModules || module instanceof ExternalModule)) {
      ...
      variable.setRenderNames(null, getSafeName([...module.exportedVariables].some(([exportedVariable, exportedName]) => exportedName === "*" && exportedVariable.included) ? module.suggestedVariableName + "__default" : module.suggestedVariableName, usedNames));
    } else {
      variable.setRenderNames(null, getSafeName(name, usedNames));
    }
  }
  ...
  for (const variable of imports) {
    ...
    if (module instanceof ExternalModule) {
      ...
      if (name === "default") {
        ...
        const variableName = defaultInteropHelpersByInteropType[moduleInterop] ? module.defaultVariableName : module.variableName;
        ...
        variable.setRenderNames(null, namespaceInteropHelpersByInteropType[String(interop(module.id))] ? module.namespaceVariableName : module.variableName);
      } else {
...
var NON_ASSET_EXTENSIONS = [".js", ".jsx", ".ts", ".tsx"];
...
var Chunk2 = class {
  ...
  canModuleBeFacade(module, exposedVariables) {
    ...
    for (const exposedVariable of this.exports) {
      if (!moduleExportNamesByVariable.has(exposedVariable)) {
        if (moduleExportNamesByVariable.size === 0 && module.isUserDefinedEntryPoint && module.preserveSignature === "strict" && this.unsetOptions.has("preserveEntrySignatures")) {
  ...
  generateFacades() {
    ...
    for (const module of entryModules) {
      ...
      for (const facadeName of requiredFacades) {
        facades.push(Chunk2.generateFacade(this.inputOptions, this.outputOptions, this.unsetOptions, this.pluginDriver, this.modulesById, this.chunkByModule, this.facadeChunkByModule, this.includedNamespaces, module, facadeName));
      }
    }
    ...
    for (let exportName of this.getExportNames()) {
      ...
      if (exportName[0] === "*") {
        ...
        if (module instanceof Module) {
          ...
          needsLiveBinding = externalLiveBindings && (imported !== "default" || isDefaultAProperty(String(interop(module.id)), true));
        }
      }
  ...
  getRelativePath(targetPath, stripJsExtension) {
    let relativePath2 = normalize(relative(dirname(this.id), targetPath));
...
var concatSep = (out, next) => next ? `${out}
${next}` : out;
var concatDblSep = (out, next) => next ? `${out}
...
function getChunkAssignments(entryModules, manualChunkAliasByEntry) {
  ...
  function assignEntryToStaticDependencies(entry, dynamicDependentEntryPoints) {
    ...
    for (const module of modulesToHandle) {
      ...
      if (dynamicDependentEntryPoints && areEntryPointsContainedOrDynamicallyDependent(dynamicDependentEntryPoints, dependentEntryPointsByModule.get(module))) {
        continue;
      } else {
...
var Bundle2 = class {
  ...
  async generateChunks() {
    ...
    for (const { alias, modules } of this.outputOptions.inlineDynamicImports ? [{ alias: null, modules: getIncludedModules(this.graph.modulesById) }] : this.outputOptions.preserveModules ? getIncludedModules(this.graph.modulesById).map((module) => ({
      alias: null,
      modules: [module]
...
function getIncludedModules(modulesById) {
  return [...modulesById.values()].filter((module) => module instanceof Module && (module.isIncluded() || module.info.isEntry || module.includedDynamicImporters.length > 0));
}
...
var readFile = (file) => new Promise((fulfil, reject) => fs.readFile(file, "utf-8", (err, contents) => err ? reject(err) : fulfil(contents)));
...
var ModuleLoader = class {
  ...
  getResolveDynamicImportPromises(module) {
    return module.dynamicImports.map(async (dynamicImport) => {
      const resolvedId = await this.resolveDynamicImport(module, typeof dynamicImport.argument === "string" ? dynamicImport.argument : dynamicImport.argument.esTreeNode, module.id);
  ...
  getResolveStaticDependencyPromises(module) {
    return Array.from(module.sources, async (source) => [
      source,
      module.resolvedIds[source] = module.resolvedIds[source] || this.handleResolveId(await this.resolveId(source, module.id, EMPTY_OBJECT), source, module.id)
    ]);
  }
  ...
  async resolveDynamicImport(module, specifier, importer) {
    ...
    if (resolution == null) {
      return module.resolvedIds[specifier] = module.resolvedIds[specifier] || this.handleResolveId(await this.resolveId(specifier, module.id, EMPTY_OBJECT), specifier, module.id);
    }
...
var getAcorn = (config) => __spreadValues({
  allowAwaitOutsideFunction: true,
  ecmaVersion: "latest",
  preserveParens: false,
  sourceType: "module"
}, config.acorn);
var getAcornInjectPlugins = (config) => ensureArray(config.acornInjectPlugins);
var getCache = (config) => {
  var _a;
...
var getMaxParallelFileReads = (config) => {
...
var getPreserveEntrySignatures = (config, unsetOptions) => {
...
var getTreeshake = (config, warn, strictDeprecations) => {
...
var getHasModuleSideEffects = (moduleSideEffectsOption, pureExternalModules) => {
...
var getDynamicImportFunction = (config, inputOptions) => {
...
var ALLOWED_INTEROP_TYPES = new Set(["auto", "esModule", "default", "defaultOnly", true, false]);
var getInterop = (config, inputOptions) => {
...
function loadFsEvents() {
  return Promise.resolve().then(() => __toModule(__require("fsevents"))).then((namespace) => {
